...
class Ui_Clause_Eng_Window(QtCore.QObject):
    def setupUi(self, MainWindow):
        super(Ui_Clause_Eng_Window, self).__init__()
        ...
        self.text_to_type = '''1. Among all the English counties, there is nothing more attractive to me than Dorset; this is where I decided to settle after retirement.
2. A few seconds later, on the left, from one of the trees that grow along the alley, the cries of a cuckoo were heard.
3. The smell of wet grass and wallflowers wafted over us. The village seemed to breathe peace and tranquility.
4. I got out of bed, went to the window and, leaning out, began to shout and clap my hands.'''
        ...
        if obj == self.VvodTexta and event.type() == QtCore.QEvent.KeyPress:
            if not self.is_paused and not self.is_exercise_completed:
                ...
                if input_text == self.text_to_type[:len(input_text)]:
                    ...
                    self.show_next_key_image()
        return super(Ui_Clause_Eng_Window, self).eventFilter(obj, event)
