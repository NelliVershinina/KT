...
class Ui_Clause_Ru_Window(QtCore.QObject):
    def setupUi(self, MainWindow):
        super(Ui_Clause_Ru_Window, self).__init__()
        ...
        self.text_to_type =  '''1. Среди всех английских графств нет для меня более привлекательного, чем Дорсет; здесь я и задумал поселиться после выхода на пенсию.
2. Через несколько секунд слева, с одного из деревьев, что растут вдоль переулка послышались крики кукушки.
3. На нас веяло запахом валжной травы и желтофиолей. Деревня, казалось, дышала миром и спокойствием.
4. Я слез с кровати, подошел к окну и, высунувшись, принялся кричать и хлопать в ладоши.'''
        ...
        if obj == self.VvodTexta and event.type() == QtCore.QEvent.KeyPress:
            if not self.is_paused and not self.is_exercise_completed:
                ...
                if input_text == self.text_to_type[:len(input_text)]:
                    ...
                    self.show_next_key_image()
        return super(Ui_Clause_Ru_Window, self).eventFilter(obj, event)
