...
class Ui_Words_Eng_Window(QtCore.QObject):
    def setupUi(self, MainWindow):
        super(Ui_Words_Eng_Window, self).__init__()
        ...
        self.text_to_type = "Letters Words Text Signs Space Vyatka State University College June Twilight Peacock Goose String Wheel Factory Heron Garage Dawn Machine Bread"
        ...
        if obj == self.VvodTexta and event.type() == QtCore.QEvent.KeyPress:
            if not self.is_paused and not self.is_exercise_completed:
                ...
                if input_text == self.text_to_type[:len(input_text)]:
                    ...
                    self.show_next_key_image()
        return super(Ui_Words_Eng_Window, self).eventFilter(obj, event)
