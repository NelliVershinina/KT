...
class Ui_Text_Ru_Window(QtCore.QObject):
    def setupUi(self, MainWindow):
        super(Ui_Text_Ru_Window, self).__init__()
        ...
        self.text_to_type = '''Задерживаться не имело смысла, тем более что я чувствовал себя разбитым - сказывалось пережитое потрясение, да и выпитое виски давало о себе знать. Элвин вызвался проводить меня. Когда мы, спотыкаясь о кочки, брели к моему дому, он нерешительно произнес:
...
- Он запустил в меня помидорами не по злобе, просто потому что перебрал.
- Угу.
...
Не слишком ли рьяно защищает Элвин своего брата? Что-то в его тоне наводило меня на подозрение, что он пытается убедить в невинности Берти не только меня, но и себя самого. Мое предположение, будто между ними существует тайный заговор, казалось мне теперь нелепым, как измышления больного ума; оно быстро улетучилось на тихом ветру, который пахнул на меня благоуханием увлажненной росой травы.'''
        ...
        if obj == self.VvodTexta and event.type() == QtCore.QEvent.KeyPress:
            if not self.is_paused and not self.is_exercise_completed:
                ...
                if input_text == self.text_to_type[:len(input_text)]:
                    ...
                    self.show_next_key_image()
        return super(Ui_Text_Ru_Window, self).eventFilter(obj, event)
