...
class Ui_Punctuation_Marks_window(QtCore.QObject):
    def setupUi(self, MainWindow):
        super(Ui_Punctuation_Marks_window, self).__init__()
        ...
        self.key_image_mapping = {
            ...
            '-': os.path.abspath('image/-.png'),
            str('/'): os.path.abspath('image/палки.png'),
            '_': os.path.abspath('image/-.png'),
            '+': os.path.abspath('image/+.png'),
            '=': os.path.abspath('image/=.png'),
            '*': os.path.abspath('image/8.png'),
            '(': os.path.abspath('image/9.png'),
            ')': os.path.abspath('image/0.png'),
            '&': os.path.abspath('image/7.png'),
            '#': os.path.abspath('image/#.png'),
            '$': os.path.abspath('image/$.png'),
            '%': os.path.abspath('image/5.png'),
            '^': os.path.abspath('image/6.png'),
            '@': os.path.abspath('image/@.png'),
        ...
        if obj == self.VvodTexta and event.type() == QtCore.QEvent.KeyPress:
            if not self.is_paused and not self.is_exercise_completed:
                ...
                if input_text == self.text_to_type[:len(input_text)]:
                    ...
                    self.show_next_key_image()
        return super(Ui_Punctuation_Marks_window, self).eventFilter(obj, event)
