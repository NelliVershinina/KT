...
class Ui_Words_Ru_Window(QtCore.QObject):
    def setupUi(self, MainWindow):
        super(Ui_Words_Ru_Window, self).__init__()
        ...
        self.text_to_type =  "Буквы Слова Текст Знаки Пробел ВятГУ Колледж ВятГУ Июнь Сумерки Павлин Гусь Вереница Колесо Фабрика Цапля Гараж Зорька Машина Хлеб"
        ...
        if obj == self.VvodTexta and event.type() == QtCore.QEvent.KeyPress:
            if not self.is_paused and not self.is_exercise_completed:
                ...
                if input_text == self.text_to_type[:len(input_text)]:
                    ...
                    self.show_next_key_image()
        return super(Ui_Words_Ru_Window, self).eventFilter(obj, event)
